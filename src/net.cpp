...
void CNode::copyStats(CNodeStats& stats)
{
    ...
    // Raw ping time is in microseconds, but show it to user as whole seconds (Dotme users should be well used to small numbers with many decimal places by now :)
...
void ThreadSocketHandler()
{
    ...
    while (true) {
        ...
        timeout.tv_sec = 0;
        timeout.tv_usec = 3000; // frequency to poll pnode->vSend
...
void ThreadMapPort()
{
    ...
    if (r == 1) {
        ...
        string strDesc = "Dotme " + FormatFullVersion();
...
bool BindListenPort(const CService& addrBind, string& strError, bool fWhitelisted)
{
    ...
    if (::bind(hListenSocket, (struct sockaddr*)&sockaddr, len) == SOCKET_ERROR) {
        ...
        if (nErr == WSAEADDRINUSE)
            strError = strprintf(_("Unable to bind to %s on this computer. Dotme Core is probably already running."), addrBind.ToString());
        else
